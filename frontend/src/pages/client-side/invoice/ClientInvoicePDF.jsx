import { Document, Page, Text, View, StyleSheet, Svg, Path, Rect, Line, Circle, Image } from "@react-pdf/renderer";
...
const getStatusStyle = (status) => {
    switch (status) {
        case 'Issued':
            return { color: '#ca8a04', backgroundColor: '#fef9c3' }; // text-yellow-500 bg-yellow-100
        case 'Partially Paid':
            return { color: '#2563eb', backgroundColor: '#dbeafe' }; // text-blue-500 bg-blue-100
        case 'Paid':
            return { color: '#16a34a', backgroundColor: '#dcfce7' }; // text-green-500 bg-green-100
        case 'Overdue':
            return { color: '#dc2626', backgroundColor: '#fee2e2' }; // text-red-500 bg-red-100
        default:
            return { color: '#6b7280', backgroundColor: '#f3f4f6' }; // text-gray-500 bg-gray-100
    }
};

const ClientInvoicePDF = ({ invoice }) => {
    const subtotal = invoice.items ? invoice.items.reduce((sum, item) => sum + (item.price * item.qty), 0) : 0;
    const discountAmount = Number(invoice.discount) || 0;
    const taxAmount = Number(invoice.tax) || 0;
    const termAmount = Number(invoice.terms?.[0]?.nominal) || 0;
    const total = subtotal - discountAmount + taxAmount - termAmount;
                ...
                <View style={styles.header}>
                    <View>
                        ...
                        {invoice.terms && invoice.terms.length > 0 && (
                            <Text style={styles.invoiceNumber}>Term {invoice.terms[0].term_number}</Text>
                        )}
                    </View>
                        ...
                        <View style={styles.summaryRow}>
                            ...
                            <Text>
                                {invoice.discount_type === 'percent'
                                    ? `${Math.round((discountAmount / subtotal) * 100)}% (Rp ${discountAmount.toLocaleString("id-ID", { minimumFractionDigits: 2 })})`
                                    : `Rp ${discountAmount.toLocaleString("id-ID", { minimumFractionDigits: 2 })}`}
                            </Text>
                        </View>
                        <View style={styles.summaryRow}>
                            ...
                            <Text>
                                {invoice.tax_type === 'percent'
                                    ? `${Math.round((taxAmount / (subtotal - discountAmount)) * 100)}% (Rp ${taxAmount.toLocaleString("id-ID", { minimumFractionDigits: 2 })})`
                                    : `Rp ${taxAmount.toLocaleString("id-ID", { minimumFractionDigits: 2 })}`}
                            </Text>
                        </View>
                        <View style={styles.summaryRow}>
                            <Text style={styles.summaryLabel}>Term {invoice.terms?.[0]?.term_number || 1}:</Text>
                            <Text>- {formatCurrency(termAmount)}</Text>
                        </View>
...
export default ClientInvoicePDF;
